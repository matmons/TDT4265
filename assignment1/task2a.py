...
def pre_process_images(X: np.ndarray):
    ...
    norm = (X/255)*2-1
    X = np.append(norm, np.ones((X.shape[0],1)), axis=1)
    return X
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    c = (1/targets.shape[0])*np.sum(-(np.transpose(targets).dot(np.log(outputs)) + np.transpose((1-targets)).dot(np.log(1-outputs))))
    return c
...
class BinaryModel:
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        self.grad = -((1/X.shape[0])*np.transpose(X).dot((targets-outputs)))
